...
public class MD5Password {
	...
	public static String MD5Encode(String inStr, String charsetname) {
		...
		try {
			...
			byte[] md5Bytes = null;//md5.digest(byteArray);
			
			if (charsetname == null || "".equals(charsetname))
				md5Bytes = md5.digest(inStr.getBytes());
			else
				md5Bytes = md5.digest(inStr.getBytes(charsetname));
	...
	public static String encryptPassword(String password) {
		return md5(md5(password) + "duoji");
	}

	// 测试主函数
	public static void main(String args[]) {
		System.out.println(RandomUtils.getRandomCharAndNumr(32));
	}
}
